...
export function observeDepositSubmission<T extends object>(adapter: T, onSubmitted: (txHash: string) => void): T {
    ...
    get(target, key) {
      ...
      if (key === "prepareAction" && typeof value === "function") {
        return async (...args: unknown[]) => {
          ...
          if (!/^gateway\.v1\.deposit(?:For|WithPermit|WithAuthorization)?$/.test(String(args[0]))) return request;

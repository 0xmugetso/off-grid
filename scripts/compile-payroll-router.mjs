...
const contractPath = path.join(projectRoot, "contracts", "PayrollRouter.sol");
const outputDirectory = path.join(projectRoot, "lib", "generated");
const outputPath = path.join(outputDirectory, "payroll-router-artifact.json");
...
const input = {
  language: "Solidity",
  sources: { "PayrollRouter.sol": { content: source } },
  settings: {
...
const artifact = compiled.contracts?.["PayrollRouter.sol"]?.PayrollRouter;
if (!artifact?.abi || !artifact?.evm?.bytecode?.object) {
  throw new Error("PayrollRouter compilation did not produce an ABI and bytecode");
}

await mkdir(outputDirectory, { recursive: true });
await writeFile(outputPath, `${JSON.stringify({
  source: "OffGrid/PayrollRouter",
  license: "MIT",
  compiler: solc.version(),
  abi: artifact.abi,
  bytecode: `0x${artifact.evm.bytecode.object}`,
}, null, 2)}\n`, "utf8");
console.log(`Compiled PayrollRouter artifact: ${path.relative(projectRoot, outputPath)}`);

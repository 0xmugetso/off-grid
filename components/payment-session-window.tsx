              ...
              <div className="session-action ready">
                ...
                {session.payerRail === "web3_usdc" && session.receiverRail === "fiat_bank" ? (
                  <>
                    ...
                    {session.actionRole === "payer" && !session.fiatSettlement && <button className="provider-action-button" disabled={settlementBusy || !fiatStatus?.configured} onClick={() => advanceSettlement()}><span>{settlementBusy ? <LoaderCircle className="spin" size={17} /> : <Wallet size={17} />}</span><div><small>START SETTLEMENT</small><b>Prepare deposit route</b></div><ArrowRight size={16} /></button>}
                    ...
                    {session.fiatSettlement && !["awaiting_web3_deposit", "complete"].includes(session.fiatSettlement.stage) && <button className="provider-action-button secondary" disabled={settlementBusy} onClick={() => advanceSettlement()}><span>{settlementBusy ? <LoaderCircle className="spin" size={17} /> : <Radio size={17} />}</span><div><small>SYNC PROVIDER PROOF</small><b>{settlementBusy ? "Checking settlement" : "Check latest progress"}</b></div><ArrowRight size={16} /></button>}
                  </>
                ) : session.payerRail === "fiat_bank" && session.receiverRail === "web3_usdc" ? (
                  <>
                    ...
                    {session.actionRole === "payer" && !session.fiatSettlement && <button className="provider-action-button" disabled={settlementBusy || !fiatStatus?.configured} onClick={() => advanceSettlement()}><span>{settlementBusy ? <LoaderCircle className="spin" size={17} /> : <Banknote size={17} />}</span><div><small>BEGIN TEST SETTLEMENT</small><b>Start sandbox bank payment</b></div><ArrowRight size={16} /></button>}
                    {session.actionRole === "payer" && session.fiatSettlement && session.fiatSettlement.stage !== "complete" && <button className="provider-action-button secondary" disabled={settlementBusy} onClick={() => advanceSettlement()}><span>{settlementBusy ? <LoaderCircle className="spin" size={17} /> : <Radio size={17} />}</span><div><small>SYNC PROVIDER PROOF</small><b>{settlementBusy ? "Checking settlement" : "Check latest progress"}</b></div><ArrowRight size={16} /></button>}
...
function ProviderRouteStatus({ status, label, settlement }: { status: { configured: boolean; checks: Array<{ key: string; label: string; configured: boolean }> } | null; label: string; settlement?: PaymentSessionView["fiatSettlement"] }) {
  ...
  const stageStates = stages.map((stage) => ({
    ...stage,
    done: !failed && Boolean(settlement) && (settlement?.stage === "complete" || order.indexOf(stage.key) <= current),
  }));
  ...
  return <div className={`provider-route-status${failed ? " failed" : ""}`}>
    <div className="provider-route-head"><span className="provider-route-icon"><Banknote size={18} /></span><div><small>TEST MODE · VERIFIED PROVIDER PROOFS</small><b>{status?.configured ? label : "Provider setup required"}</b><em>{routeState}</em></div><strong>{completedCount}<span>/4</span><small> VERIFIED</small></strong></div>
    {settlement ? <>
      <div className="provider-progress-track" aria-hidden="true"><span style={{ width: `${Math.max(4, completedCount * 25)}%` }} /></div>
      <div className="provider-proof-steps">{stageStates.map((stage, index) => { const active = index === activeIndex; const proof = stage.proof ? `${stage.proof.slice(0, 10)}…${stage.proof.slice(-6)}` : null; return <div className={stage.done ? "done" : active ? "active" : "upcoming"} key={stage.key}><i>{stage.done ? <Check size={11} /> : active ? <Radio size={10} /> : index + 1}</i><span><small>STEP 0{index + 1}</small><b>{stage.label}</b><em>{stage.done ? "Proof verified" : active ? isWaitingForPayer ? "Waiting to start" : "In progress" : "Queued"}</em>{proof && (stage.href ? <a href={stage.href} target="_blank" rel="noreferrer" title={stage.proof || undefined}>{proof} <ExternalLink size={10} /></a> : <code title={stage.proof || undefined}>{proof}</code>)}</span></div>; })}</div>
      {web3ToFiat && settlement.circlePayoutStatus ? <p className="provider-proof-note">Circle payout <b>{settlement.circlePayoutStatus}</b>{settlement.circlePayoutTrackingRef ? ` · ${settlement.circlePayoutTrackingRef}` : ""}</p> : settlement.circleDepositStatus ? <p className="provider-proof-note">Circle deposit <b>{settlement.circleDepositStatus}</b>{settlement.circleDepositAmount ? ` · ${settlement.circleDepositAmount} USD` : ""}{settlement.arcBlockNumber ? ` · block ${settlement.arcBlockNumber}` : ""}</p> : null}

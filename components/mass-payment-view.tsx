...
import { ArrowRight, Check, ChevronDown, CircleAlert, CircleCheck, ExternalLink, Network, Plus, Search, Send, ShieldCheck, Trash2, Upload, UserPlus, Users, Wallet, X, Zap } from "lucide-react";
...
export function MassPaymentView({ walletAddress, directBalance, unifiedBalance, onConnect, onExecute, onBackToDashboard }: { walletAddress: string; directBalance: string | null; unifiedBalance: string | null; onConnect: () => void; onBackToDashboard: () => void; onExecute: (members: MassTeamMember[], funding: MassFunding, onProgress: (completed: number, total: number) => void) => Promise<MassRunResult> }) {
  ...
  async function execute() {
    if (!walletAddress || !amountsValid || (insufficient && !error)) return;
  ...
  return <section className="mass-payment-view">
    <div className="mass-heading"><div><span className="section-tag">TEAM OPERATIONS</span><h1>Pay everyone.<br /><em>In one run.</em></h1><p>Build a clean team roster, choose equal or individual pay, then settle through your direct wallet or Unified Balance.</p></div><div className="mass-flow"><span className={members.length ? "done" : "active"}><i>{members.length ? <Check size={10} /> : "1"}</i><b>Team</b><small>Add recipients</small></span><em /><span className={amountsValid ? "done" : members.length ? "active" : ""}><i>{amountsValid ? <Check size={10} /> : "2"}</i><b>Amounts</b><small>Set allocation</small></span><em /><span className={amountsValid ? "active" : ""}><i>3</i><b>Settle</b><small>Review & sign</small></span></div></div>
    ...
    {!walletAddress ? <div className="mass-connect"><Users size={33} /><h2>Connect your treasury wallet first.</h2><p>Your wallet remains the signer. OffGrid never takes custody of payroll funds.</p><button className="neon-button" onClick={onConnect}><Wallet size={15} /> Connect wallet</button></div> : <div className="mass-layout">
      ...
      <aside className="mass-summary"><span className="section-tag">03 · SETTLEMENT</span><h2>Payroll summary</h2><div className="mass-total"><small>TOTAL PAYROLL</small><b>{money(total)} <em>USDC</em></b><p>{members.length} recipient{members.length === 1 ? "" : "s"} · {allocation === "equal" ? "equal allocation" : "custom allocation"}</p></div><label className="funding-label">PAY FROM</label><div className="mass-funding"><button className={funding === "unified_balance" ? "active" : ""} onClick={() => setFunding("unified_balance")}><Network size={16} /><span><b>Unified Balance</b><small>{money(unifiedBalance ?? 0)} USDC confirmed</small></span>{funding === "unified_balance" && <Check size={13} />}</button><button className={funding === "arc_wallet" ? "active" : ""} onClick={() => setFunding("arc_wallet")}><Wallet size={16} /><span><b>Direct wallet</b><small>{money(directBalance ?? 0)} USDC available</small></span>{funding === "arc_wallet" && <Check size={13} />}</button></div><div className="mass-route-note"><Zap size={14} /><div><b>{funding === "arc_wallet" ? "One approval · One payout" : "One Gateway authorization per source wallet"}</b><p>{funding === "arc_wallet" ? "Approve the payroll total, then send every payout in one Arc transaction." : "Authorize the total once per source wallet. The Gateway mint and all payouts settle together on Arc."}</p></div></div><dl><div><dt>Recipients</dt><dd>{members.length}</dd></div><div><dt>Available</dt><dd>{money(available)} USDC</dd></div><div><dt>Payroll</dt><dd>{money(total)} USDC</dd></div><div><dt>Remaining</dt><dd className={insufficient ? "negative" : ""}>{money(available - total)} USDC</dd></div></dl>{error && <p className="mass-error"><CircleAlert size={13} />{error}</p>}<button className="mass-review-button" disabled={!amountsValid || insufficient} onClick={() => { setError(""); setResult(null); setShowReview(true); }}><span><Send size={17} /></span><div><small>FINAL CHECK</small><b>{insufficient ? "Insufficient balance" : `Review ${members.length} payouts`}</b></div><ArrowRight size={16} /></button><p className="mass-custody"><ShieldCheck size={12} /> You approve every onchain action in your wallet.</p></aside>
    </div>}

    {showReview && createPortal(<dialog ref={reviewRef} className="mass-review-dialog" aria-label="Review payroll run" onCancel={(event) => { if (busy) event.preventDefault(); else setShowReview(false); }}><article className="mass-review-modal"><button className="modal-x" onClick={() => !busy && setShowReview(false)} disabled={busy}><X size={18} /></button>{result ? <div className={`mass-success ${result.partial ? "partial" : ""}`}><span>{result.partial ? <CircleAlert size={25} /> : <CircleCheck size={25} />}</span><small>{result.partial ? "PAYROLL RUN STOPPED" : "PAYROLL RUN COMPLETE"}</small><h2><span className="mass-heading-number">{result.txHashes.length}</span> of <span className="mass-heading-number">{members.length}</span> payments submitted.</h2><p>{result.partial ? `Completed transfers were saved and must not be submitted again. ${result.errorMessage ?? "The remaining payouts were not executed."}` : result.mode === "router_batch" || result.mode === "gateway_batch" ? "All recipients were paid in one atomic Arc transaction." : result.mode === "wallet_batch" ? "Your wallet executed the transfers as an EIP-5792 batch." : result.mode === "gateway_sequential" ? "Circle Gateway settled each recipient on Arc Testnet." : "The wallet completed each transfer sequentially."}</p><div><span><small>TRANSACTIONS</small><b>{new Set(result.txHashes).size}</b></span><span><small>RECEIPTS SAVED</small><b>{result.receiptsSaved}/{result.txHashes.length}</b></span><span><small>SETTLED</small><b>{result.txHashes.length}/{members.length}</b></span></div><ul className="mass-result-list" aria-label="Submitted payments">
          {payableMembers.slice(0, result.txHashes.length).map((member, index) => <li key={member.id}>
            <span><b>{member.name}</b><small title={member.address}>{shortAddress(member.address)}</small></span>
            <strong>{money(member.amount)} <small>USDC</small></strong>
            {result.explorerUrls[index] && <a href={result.explorerUrls[index]} target="_blank" rel="noreferrer" aria-label={`View transaction for ${member.name}`} title="View transaction"><ExternalLink size={15} /></a>}
          </li>)}
        </ul><button className="neon-button" onClick={() => { setShowReview(false); if (!result.partial) setMembers([]); setResult(null); onBackToDashboard(); }}>Back to dashboard <ArrowRight size={15} /></button></div> : <><span className="section-tag">REVIEW PAYROLL RUN</span><h2><span className="mass-heading-number">{money(total)}</span> USDC to <span className="mass-heading-number">{members.length}</span> wallets.</h2><p>Confirm the final manifest. Onchain transfers cannot be edited or reversed after signing.</p><div className="mass-review-list">{payableMembers.map((member) => <div key={member.id}><span><b>{member.name}</b><small>{shortAddress(member.address)}</small></span><strong>{money(member.amount)} USDC</strong></div>)}</div><div className="mass-review-route"><span>{funding === "arc_wallet" ? <Wallet size={15} /> : <Network size={15} />}</span><div><small>SETTLEMENT ROUTE</small><b>{funding === "arc_wallet" ? "Direct wallet batch" : "Circle Unified Balance"}</b></div><em>{members.length} PAYOUTS</em></div>{busy && <div className="mass-progress"><div><span style={{ width: `${progress.total ? (progress.completed / progress.total) * 100 : 8}%` }} /></div><p><LoaderCircle className="spin" size={13} /> {progress.completed ? `${progress.completed} of ${progress.total} confirmed` : "Waiting for wallet approval…"}</p></div>}{error && <p className="mass-error"><CircleAlert size={13} />{error}</p>}<button className="neon-button mass-execute" onClick={execute} disabled={busy}>{busy ? <LoaderCircle className="spin" size={16} /> : <Zap size={16} />}<span>{busy ? "Payroll is settling…" : "Approve & execute payroll"}</span>{!busy && <ArrowRight size={15} />}</button><small className="mass-final-note"><ShieldCheck size={11} /> Recipient wallets and amounts are validated before execution.</small></>}</article></dialog>, document.body)}
  </section>;
}

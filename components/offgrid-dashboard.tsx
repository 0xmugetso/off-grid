...
function EscrowView({ walletAddress, arcBalance, onConnect, onRefresh }: { walletAddress: string; arcBalance: string | null; onConnect: () => void; onRefresh: () => void }) {
  ...
  return <section className="escrow-view">
    <div className="view-heading escrow-market-heading"><div><span className="section-tag">REFUNDPROTOCOL · ARC TESTNET</span><h1>Escrow Marketplace</h1><p>Post protected work, accept verified jobs, and settle completed work on Arc.</p><div className="escrow-heading-guide"><span><b>1</b> Post the scope and budget</span><i/><span><b>2</b> A verified worker accepts</span><i/><span><b>3</b> Fund, verify, and settle on Arc</span></div></div><div className="escrow-live-actions"><button className="escrow-workspace-link" onClick={() => setEscrowSection("mine")}><BriefcaseBusiness size={15}/><span><b>My escrows</b><small>{activeEscrows.length} active · {historyEscrows.length} completed</small></span></button><button className="neon-button" onClick={() => walletAddress ? setShowCreateModal(true) : onConnect()}><Plus size={15}/> {walletAddress ? "Post a job" : "Connect wallet"}</button></div></div>
    ...
    {activeEscrow && <div className="overlay escrow-inspector-overlay"><article className="history-proof-modal escrow-inspector">
      ...
      <section className="escrow-inspector-section escrow-route-section"><div className="escrow-inspector-section-head"><span className="section-tag">ONCHAIN ROUTE</span><small>Each available node opens in ArcScan</small></div><div className="escrow-route-line">{inspectorRouteNodes.map((node, index) => <div className="escrow-route-node-wrap" key={node.label}>{index > 0 && <ArrowRight className="escrow-route-arrow" size={14}/>}<a href={node.address ? `https://testnet.arcscan.app/address/${node.address}` : undefined} target="_blank" rel="noreferrer" className={!node.address ? "disabled" : ""}>{node.icon}<span><small>{node.label}</small><code>{node.address ? shortAddress(node.address, 5) : "Pending"}</code></span>{node.address && <ExternalLink size={11}/>}</a></div>)}</div><p className="escrow-route-help">The creator funds a Circle smart account. RefundProtocol locks the payment, then an approved settlement moves it through the worker smart account to the connected wallet.</p></section>

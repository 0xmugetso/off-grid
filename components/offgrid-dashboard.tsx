...
import { ArcPayrollClient, supportsFastDeposit, type DepositEstimate, getArcMintStep, type BrowserSolanaAdapter, type BrowserViemAdapter, type CircleAdapter, type GatewayMintRetry } from "@/lib/arc/app-kit-client";
...
type PaymentEstimate = { title: string; detail: string; fees: string; bridgeQuote?: string };
...
function fiatProofEvents(session: PaymentSessionView): ProtocolEvent[] {
  ...
  return [
    ...
    { name: settlement.circleDepositId ? `Circle deposit verified · ${settlement.circleDepositId}` : "Waiting for Circle deposit", state: settlement.circleDepositStatus && /fail|denied|cancel/i.test(settlement.circleDepositStatus) ? "failed" : settlement.circleDepositId && /^(complete|completed|confirmed)$/i.test(settlement.circleDepositStatus || "") ? "confirmed" : "pending" },
...
function gatewayDepositPresentation(deposit: GatewayDeposit): GatewayDepositPresentation {
  ...
  if (deposit.mode === "fast") return { label: "Depositing on Arc", detail: "Circle is completing the deposit into your Arc unified balance.", delayed: elapsedMs > 120_000, elapsed };
...
function gatewayDepositLedgerEntry(deposit: GatewayDeposit): LedgerEntry {
  ...
  return {
    ...
    kind: "deposit",
    rail: `${CHAIN_LABELS[deposit.sourceChain as SourceChain] ?? deposit.sourceChain} → ${deposit.mode === "fast" ? "Arc Gateway" : "Gateway"}`,
    ...
    logs: [
      ...
      { name: deposit.status === "confirmed" ? "Circle Gateway Balance Credited" : gatewayDepositPresentation(deposit).label, ...(deposit.destinationTxHash ? { txHash: deposit.destinationTxHash, explorerUrl: gatewayExplorerUrl("Arc_Testnet", deposit.destinationTxHash) } : {}) },
    ],
  };
}
...
function UnifiedBalanceView({ deposits, walletAddress, walletOnArc, arcBalance, unifiedBalance, pendingBalance, chainBalances, gatewayError, gatewayStale, gatewayLoading, solanaAddress, solanaWalletName, solanaUsdcBalance, solanaBusy, onRefresh, onDeposit, onTrackDeposits, hasPendingDeposits, onConnect, onConnectSolana }: { deposits: GatewayDeposit[]; walletAddress: string; walletOnArc: boolean; arcBalance: string | null; unifiedBalance: string | null; pendingBalance: string | null; chainBalances: GatewayChainBalance[] | null; gatewayError: string; gatewayStale: boolean; gatewayLoading: boolean; solanaAddress: string; solanaWalletName: string; solanaUsdcBalance: string | null; solanaBusy: boolean; onRefresh: () => void; onDeposit: () => void; onTrackDeposits: () => void; hasPendingDeposits: boolean; onConnect: () => void; onConnectSolana: () => void }) {
  ...
  return <section className="unified-view">
    ...
    {!walletAddress ? <div className="unified-empty"><Network size={30} /><h2>Connect a wallet to query Gateway.</h2><p>OffGrid will read only real confirmed and pending balances for your connected address.</p><button className="neon-button" onClick={onConnect}><Wallet size={15} /> Connect Wallet</button></div> : <>
      ...
      <div className="unified-chain-head"><div><span className="section-tag">SOURCE ALLOCATION</span><h2>Balance by chain</h2><p>Your Gateway balance, grouped by deposit source.</p></div><span><i /> CONFIRMED <i /> PENDING</span></div>
      <div className="unified-chain-grid">{positions.map((position) => {
        const fastDeposited = deposits.filter((deposit) => deposit.mode === "fast" && deposit.status === "confirmed" && deposit.sourceChain === position.chain && deposit.sourceAddress.toLowerCase() === walletAddress.toLowerCase()).reduce((total, deposit) => total + Number(deposit.amount), 0);
        const allFastDeposited = deposits.filter((deposit) => deposit.mode === "fast" && deposit.status === "confirmed" && deposit.sourceAddress.toLowerCase() === walletAddress.toLowerCase()).reduce((total, deposit) => total + Number(deposit.amount), 0);
        const arcConfirmed = Number(positions.find((entry) => entry.chain === "Arc_Testnet")?.confirmed ?? 0);
        const attributedFastBalance = Math.min(allFastDeposited, arcConfirmed);
        const sourceShare = allFastDeposited > 0 ? attributedFastBalance * fastDeposited / allFastDeposited : 0;
        const displayedBalance = position.chain === "Arc_Testnet" ? arcConfirmed - attributedFastBalance : Number(position.confirmed) + sourceShare;
        const chainConfirmed = displayedBalance;
        ...
        return <article className={!position.queried ? "unlinked" : chainPending > 0 ? "indexing" : ""} key={position.chain}>
          <div className="unified-chain-logo"><ChainLogo chain={position.chain} size={33}/>{supportsFastDeposit(position.chain) && <span className="fast-deposit-hint" tabIndex={0} aria-label="About fast deposit balances" aria-describedby={`fast-tip-${position.chain}`}><Zap size={13} /><span className="fast-deposit-tooltip" role="tooltip" id={`fast-tip-${position.chain}`}>Fast deposits move your funds to Arc Gateway. This number includes the Arc-held balance attributed to this source; it is counted only once in your total. After spending, source attribution is an estimate.</span></span>}</div>
          <div><small>{CHAIN_LABELS[position.chain].toUpperCase()}</small><b>{position.queried ? displayMoney(displayedBalance) : "-"} <em>USDC</em></b><p>{!position.queried ? "Connect Solana Wallet To Query" : chainPending > 0 ? `${displayMoney(position.pending)} USDC awaiting finality` : "Gateway confirmed"}</p></div>
      ...
      <div className="unified-eta-row"><span><Clock size={12} /> Deposit ETA</span><span>Ethereum / Arbitrum <b>Fast · seconds</b></span><span>Base <b>~13–19 min</b></span><span>Solana <b>~8 sec</b></span><span>Arc <b>~0.5 sec</b></span></div>
...
function EscrowView({ walletAddress, arcBalance, onConnect, onRefresh }: { walletAddress: string; arcBalance: string | null; onConnect: () => void; onRefresh: () => void }) {
  ...
  useEffect(() => {
    const sync = () => { if (!document.hidden) void loadEscrows(true); };
    const timer = window.setInterval(sync, hasPendingAction ? 4000 : 12000);
  ...
  const renderEscrowActions = (item: EscrowItem) => <>
    {item.lastError && <p className="escrow-card-error" role="alert">{item.lastError}</p>}
    {item.status === "initiated" && isDepositor(item) && item.providerUserId && !configuration?.configured && <p className="escrow-card-error" role="alert">Escrow deployment is unavailable. Check the Circle escrow configuration before retrying.</p>}
  ...
  return <section className="escrow-view">
    ...
    {activeEscrow && <div className="overlay escrow-inspector-overlay"><article className="history-proof-modal escrow-inspector">
      ...
      <div className="escrow-inspector-actions">
        {actionError && <p className="escrow-card-error" role="alert">{actionError}</p>}
        {renderEscrowActions(activeEscrow)}
      </div>
...
export function OffGridDashboard() {
  ...
  const [depositQuote, setDepositQuote] = useState<{ chain: SourceChain; amount: string; estimate: DepositEstimate } | null>(null);
  const [depositNotice, setDepositNotice] = useState<{ chain: SourceChain; hash: string; fast?: boolean; error?: string } | null>(null);
  ...
  const fastDepositInFlight = gatewayDeposits.some((deposit) => deposit.mode === "fast" && deposit.sourceChain === depositChain && deposit.status !== "confirmed" && (deposit.status !== "failed" || Boolean(deposit.sourceConfirmedAt)));
  ...
  async function refreshCctpOperations() {
    try {
      ...
      const terminal = operations.find((operation) => isSubmittedCctpOperation(operation) && (operation.status === "confirmed" || operation.status === "failed") && previouslyPending.has(operation.id));
      if (terminal) {
  ...
  async function captureCctpBurn(operationId: string, txHash: string, explorerUrl?: string) {
    ...
    try {
      ...
      setCctpRecoveryNote(run.sourceChain === "Solana_Devnet" ? "Source burn submitted. Confirm the Arc mint in your EVM wallet when prompted." : "Source burn submitted. Circle attestation and the Arc mint will continue here automatically.");
  ...
  async function depositToGateway() {
    if (depositBusy || fastDepositInFlight) return;
    ...
    try {
      ...
      const fast = supportsFastDeposit(depositChain);
      const quoted = depositQuote?.chain === depositChain && depositQuote.amount === depositAmount ? depositQuote.estimate : undefined;
      if (fast && !quoted) {
        const estimate = await client.estimateDeposit(adapter, depositChain, depositAmount);
        setDepositQuote({ chain: depositChain, amount: depositAmount, estimate });
        return;
      }
      ...
      const saveDeposit = async (txHash: string) => {
        const { deposit } = await api<{ deposit: GatewayDeposit }>("/api/gateway-deposits", {
          method: "POST",
          body: JSON.stringify({ sourceAddress: depositChain === "Solana_Devnet" ? solanaAddress : (walletAddress || user?.walletAddress), sourceChain: depositChain, amount: depositAmount, txHash, confirmedBefore, mode: fast ? "fast" : "standard" }),
        });
      ...
      const onSubmitted = (txHash: string) => {
        ...
        setDepositNotice({ chain: depositChain, fast, hash: txHash });
        tracking = saveDeposit(txHash).catch(() => {
          setDepositNotice({ chain: depositChain, fast, hash: txHash, error: "Deposit sent. History could not be updated yet. Keep this transaction reference and check the explorer before retrying." });
        });
      };
      const result = await client.deposit(adapter, depositChain, depositAmount, onSubmitted, quoted);
      if (!submittedHash && !(fast && result.progress?.status === "DONE")) onSubmitted(result.txHash);
      ...
      if (submittedHash) {
        ...
        void refreshGatewayDeposits();
      } else { setDepositQuote(null); setDepositError(describeGatewayDepositIssue(error instanceof Error ? error.message : "Gateway deposit failed", depositChain)); }
    }
  ...
  async function estimatePayment() {
    ...
    try {
      ...
      if (fundingMethod === "arc_wallet") {
        ...
        const estimate = await client.estimateBridgeToArc(sourceAdapter, bridgeSourceChain, recipientAddress, amount, adapter);
        setPaymentEstimate({ title: `${CHAIN_LABELS[bridgeSourceChain]} → Arc Testnet`, detail: bridgeSourceChain === "Solana_Devnet" ? "Confirm the source burn in Solana, then the Arc mint in your EVM wallet" : "CCTP V2 · recipient receives the entered amount; fees paid on source", fees: bridgeFeeSummary(estimate), bridgeQuote: "quote" in estimate ? String(estimate.quote) : undefined });
      }
    } catch (error) {
  ...
  async function pay() {
    ...
    if (fundingMethod === "cctp_bridge") {
      ...
      try {
        ...
        setCctpOperations((current) => [operation!, ...current.filter((entry) => entry.id !== operation!.id)]);
        let result = await client.bridgeToArc(sourceAdapter, bridgeSourceChain, recipientAddress, amount, operation.id, paymentEstimate.bridgeQuote, adapter);
        if (result.state === "error") result = await client.retryBridge(result, sourceAdapter, bridgeSourceChain === "Solana_Devnet" ? adapter : undefined);
  ...
  return (
    <main className="product-shell">
              ...
              <section className="pay-console" id="payment-console">
                ...
                <div className="console-body">
                  ...
                  <aside className="route-panel">
                    ...
                    </> : step === "processing" ? <><div className="protocol-progress">{(fundingMethod === "fiat_bank" ? [["estimate","Route check"],["settlement","Provider proofs"],["receipt","Onchain receipt"]] : [["estimate","Live estimate"],["signature","Wallet signature"],["settlement",fundingMethod === "cctp_bridge" ? "CCTP lifecycle" : "Network confirmation"],["receipt","Create receipt"]]).map(([phase,label], index, phases) => { const current = phases.findIndex(([name]) => name === paymentPhase); return <span key={phase} className={index <= current ? "active" : ""}><i>{index < current ? <Check size={9} /> : index + 1}</i>{label}</span>; })}</div>{protocolEvents.length > 0 && <div className="protocol-stream">{protocolEvents.map((event, index) => {
              ...
              {proofSteps(selectedProofEntry).map((step, index) => (
                <section key={`${step.label}-${index}`} className={`history-proof-step ${step.tone}`}>
                  ...
                  <div>
                    <b>{step.label}</b>
                    <p>{step.detail}</p>
                    {step.txHash && <div className="history-proof-reference"><code>{step.txHash}</code>{step.explorerUrl && <a href={step.explorerUrl} target="_blank" rel="noreferrer" aria-label={`Open ${step.label} in explorer`}><ExternalLink size={11} /></a>}</div>}
                  </div>
                  {selectedProofEntry.kind === "fiat" && step.label === "Circle Deposit" && <button className="history-step-refresh" onClick={() => { void refreshFiatPayouts(); }} aria-label="Refresh Circle deposit status" title="Refresh Circle deposit status"><RefreshCw size={14} /></button>}
                  {step.label.endsWith("Receipt") && selectedProofEntry.receiptUrl && <a className="history-step-receipt" href={selectedProofEntry.receiptUrl}><Receipt size={12} /> Open Receipt</a>}
                </section>
              ))}
            </div>
          </article>
                          ...
                          <div className="live-session-card" key={sess.id}>
                            <div className="session-card-header">
                              <div className="session-title-group">
                                ...
                                <h3>${sess.amount} <span style={{ fontFamily: "var(--mono)" }}>USD</span></h3>
      ...
      {showFunding && <div className="overlay"><article className="funding-modal">
        ...
        <div className="modal-chain-field"><span>Source network</span><ChainSelect value={depositChain} chains={SOURCE_CHAINS} onChange={(chain) => { setDepositChain(chain); setDepositQuote(null); setDepositError(""); }} /></div>
        ...
        <label>Amount<div className="fund-amount"><input value={depositAmount} onChange={(event) => { setDepositAmount(event.target.value.replace(/[^0-9.]/g, "")); setDepositQuote(null); setDepositError(""); }} inputMode="decimal" /><span>USDC</span></div></label>
        ...
        {fastDepositInFlight && <button className="quiet-refresh" onClick={() => { setShowFunding(false); openWorkspaceView("history"); }}><Receipt size={13} /> Track Deposits</button>}
        {gatewayDeposits.some((deposit) => deposit.sourceChain === depositChain && deposit.status !== "confirmed" && deposit.status !== "failed") && <div className="funding-background-note"><Radio size={14}/><div><b>Another deposit is still tracking</b><p>{fastDepositInFlight ? "Follow the existing deposit in History while it finishes." : "You can submit this one safely. Both transactions will remain in History with independent status."}</p></div></div>}
        {depositQuote && depositQuote.chain === depositChain && depositQuote.amount === depositAmount && <div className="deposit-fee-review"><b>Fast deposit → Arc Testnet</b><span>Deposit <strong>{depositAmount} USDC</strong></span>{depositQuote.estimate.fees.map((fee, index) => <span key={index}>{fee.type === "gasFee" ? "Estimated gas" : "Forwarder fee"}<strong>{fee.amount} {fee.token}</strong></span>)}<small>Fees are paid in addition to your deposit.</small></div>}
        <button className="neon-button" onClick={depositToGateway} disabled={depositBusy || fastDepositInFlight || !walletAddress || !(Number(depositAmount) > 0) || (depositChain === "Solana_Devnet" && !solanaAddress)}>{depositBusy ? <LoaderCircle className="spin" size={17} /> : <ArrowDownToLine size={17} />} {fastDepositInFlight ? "Track your existing deposit" : depositBusy ? depositSubmitted ? "Deposit submitted · tracking confirmation…" : supportsFastDeposit(depositChain) && !depositQuote ? "Estimating fees…" : "Confirm in wallet…" : supportsFastDeposit(depositChain) && !depositQuote ? "Review fast deposit" : "Review deposit in wallet"}</button>
        ...
        <div className="gateway-finality-note"><ShieldCheck size={15}/><div><b>Expected Credit Time</b><p>{supportsFastDeposit(depositChain) ? "Fast deposit to Arc Testnet. Typical credit: seconds after the source burn." : <>{gatewayFinalityEstimate(depositChain).detail} Expected wait: <strong>{gatewayFinalityEstimate(depositChain).time}</strong>.</>}</p></div></div>
      </article></div>}

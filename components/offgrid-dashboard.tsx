...
function CreateEscrowModal({ onClose, onCreated }: { onClose: () => void; onCreated: (item: EscrowItem) => void; walletAddress: string }) {
  ...
  const labels = ["Beneficiary", "Agreement", "Review & create"];
  return <div className="overlay escrow-create-overlay"><article className="escrow-wizard-modal escrow-wizard-premium"><button className="modal-x" onClick={onClose} aria-label="Close modal"><X size={18}/></button><header className="escrow-wizard-head"><span className="section-tag">ARC TESTNET · REFUNDPROTOCOL</span><h2>Create protected work.</h2><p>Set the counterparty, let OffGrid structure the agreement, then review exactly what will be enforced before anything touches a wallet.</p></header><div className="escrow-stepper premium-stepper">{labels.map((label, index) => <div key={label} className={`stepper-step ${step >= index + 1 ? "active" : ""} ${step > index + 1 ? "complete" : ""}`}><span>{step > index + 1 ? <Check size={12}/> : index + 1}</span><small>{label}</small></div>)}</div><form className="escrow-wizard-body premium-wizard-body" onSubmit={(event) => { event.preventDefault(); if (step === 1 && providerAddress) setStep(2); else if (step === 3 && terms) void submit(); }}>
    {step === 1 && <div className="wizard-step-pane"><div className="wizard-pane-title"><UserRound size={19}/><div><b>Select the beneficiary</b><small>Only verified OffGrid accounts can join an escrow.</small></div></div><label className="wizard-label"><span>SEARCH PEOPLE</span><div className="wizard-search-wrap"><Search size={16}/><input className="wizard-input" value={query} onChange={(event) => { setQuery(event.target.value); setProviderAddress(""); }} placeholder="Name or @username" autoFocus /></div></label><div className="escrow-recipient-results">{providers.map((provider) => <button type="button" className={`escrow-recipient-option ${providerAddress === provider.walletAddress ? "selected" : ""}`} key={provider.id} onClick={() => { setProviderAddress(provider.walletAddress); setQuery(provider.displayName); setProviders([]); }}><span className="recipient-avatar"><UserRound size={17}/></span><span><b>{provider.displayName}</b><small>@{provider.username} · {shortAddress(provider.walletAddress,6)}</small></span><CheckCircle2 size={16}/></button>)}</div><div className="wizard-route-preview"><span><Wallet size={15}/> Creator wallet</span><ArrowRight size={14}/><span><Scale size={15}/> RefundProtocol</span><ArrowRight size={14}/><span><UserRound size={15}/> Beneficiary</span></div></div>}
    {step === 2 && <div className="wizard-step-pane"><div className="wizard-pane-title"><FileCheck size={19}/><div><b>Upload the agreement</b><small>We extract the amount, deliverables, dates and validation rules.</small></div></div><label className={`escrow-dropzone ${busy ? "busy" : ""}`}><input type="file" accept=".pdf,.docx,image/png,image/jpeg,image/webp" disabled={busy} onChange={(event) => { const selected = event.target.files?.[0]; if (selected) void analyze(selected); }}/><span className="dropzone-icon">{busy ? <LoaderCircle className="spin" size={24}/> : <ArrowUpRight size={24}/>}</span><b>{busy ? "Structuring your agreement…" : file?.name || "Drop or choose an agreement"}</b><small>{busy ? "This normally takes a few seconds. Keep this window open." : "PDF, DOCX, PNG, JPG or WEBP · up to 10 MB"}</small></label>{busy && <div className="escrow-upload-progress" role="status"><span><LoaderCircle className="spin" size={14}/> AI document analysis</span><i><b/></i><small>Extracting terms and preparing the review screen</small></div>}</div>}
    {step === 3 && terms && <div className="wizard-step-pane"><div className="wizard-pane-title"><ShieldCheck size={19}/><div><b>Review the protected terms</b><small>Edit anything before the agreement becomes a live escrow.</small></div></div><div className="escrow-review-grid"><label className="wizard-label wide"><span>AGREEMENT TITLE</span><input className="wizard-input" value={terms.title} onChange={(event) => setTerms({ ...terms, title: event.target.value })}/></label><label className="wizard-label"><span>ESCROW VALUE</span><div className="wizard-amount-input"><input value={terms.amount} onChange={(event) => setTerms({ ...terms, amount: event.target.value.replace(/[^0-9.]/g, "") })}/><span>USDC</span></div></label><div className="escrow-review-metric"><small>DELIVERABLES</small><b>{terms.tasks?.length || 0}</b></div><label className="wizard-label wide"><span>AI ACCEPTANCE CRITERIA</span><textarea className="wizard-textarea" value={terms.criteria} onChange={(event) => setTerms({ ...terms, criteria: event.target.value })} rows={4}/></label></div><div className="wizard-funding-note"><Zap size={16}/><span><b>You fund it from your connected Arc wallet</b><small>After deployment, OffGrid sends the exact amount to your Circle SCA and continues the official approve → pay sequence. Refunds return to your connected wallet.</small></span></div></div>}
    {error && <p className="inline-error"><CircleAlert size={13}/> {error}</p>}<footer className="wizard-actions"><button type="button" className="wizard-back-btn" disabled={step === 1 || busy} onClick={() => setStep((value) => (value - 1) as 1 | 2 | 3)}>Back</button><span className="wizard-step-count">STEP {step} / 3</span>{step === 1 ? <button type="submit" className="neon-button" disabled={!providerAddress}>Continue <ArrowRight size={15}/></button> : step === 2 ? <span/> : <button type="submit" className="neon-button wizard-deploy-btn" disabled={busy || !terms?.criteria || Number(terms?.amount) <= 0}>{busy ? <LoaderCircle className="spin" size={16}/> : <Scale size={16}/>} Create protected agreement</button>}</footer>
  </form></article></div>;
}
...
function EscrowView({ walletAddress, arcBalance, onConnect, onRefresh }: { walletAddress: string; arcBalance: string | null; onConnect: () => void; onRefresh: () => void }) {
  ...
  const handleAction = async (item: EscrowItem, action: "deploy" | "fund" | "refund" | "refresh" | "complete_payout") => {
  ...
  const fundFromCreatorWallet = async (item: EscrowItem) => {
    ...
    try {
      ...
      const receipt = await createPublicClient({ chain: arcTestnet, transport: http(ARC.rpcUrl) }).waitForTransactionReceipt({ hash }); if (receipt.status !== "success") throw new Error("The Arc funding transaction reverted");
  ...
  const visibleEscrows = escrows.slice(0, visibleCount);
  const loadMore = () => {
    setLoadingMore(true);
  ...
  return <section className="escrow-view">
    ...
    <div className="escrow-list-panel"><div className="ledger-toolbar"><div><b>RefundProtocol agreements</b><small>{escrows.length} participant agreement{escrows.length === 1 ? "" : "s"} · live Circle transaction status</small></div>{escrows.length > 0 && <span className="escrow-list-count">SHOWING {Math.min(visibleCount, escrows.length)} / {escrows.length}</span>}</div>
      {loading ? <div className="escrow-empty-state"><LoaderCircle className="spin" size={25}/><h3>Loading escrow state</h3><p>Reading agreements and Circle transaction status.</p></div> : escrows.length === 0 ? <div className="escrow-empty-state"><div className="escrow-empty-icon"><Scale size={28}/></div><h3>No escrow agreements yet</h3><p>Create terms with another registered OffGrid user, then deploy Circle&apos;s RefundProtocol on Arc Testnet.</p><button className="neon-button" onClick={() => walletAddress ? setShowCreateModal(true) : onConnect()}><Plus size={15}/> {walletAddress ? "Create escrow agreement" : "Connect wallet"}</button></div> : <><div className="escrow-cards-grid">{visibleEscrows.map((item) => <article key={item.id} className={`escrow-item-card ${item.status}`}>
        ...
        <div className="escrow-card-copy"><h3>{item.title}</h3><p className="escrow-specs-text">{item.terms?.summary || item.specs}</p>{item.terms?.tasks?.length ? <div className="escrow-task-strip"><small>{item.terms.tasks.length} DELIVERABLE{item.terms.tasks.length === 1 ? "" : "S"}</small><span>{item.terms.tasks[0]?.description}</span></div> : null}</div>
        ...
        {item.status === "closed" && (item.beneficiaryPayoutTransactionId || item.beneficiaryPayoutTxHash) && <div className="escrow-success-banner"><CheckCircle2 size={15}/> Beneficiary wallet received the Arc payout</div>}{item.status === "refunded" && <div className="escrow-success-banner"><Unlock size={15}/> Funds returned to the depositor wallet</div>}
      </article>)}</div>{visibleCount < escrows.length && <div className="escrow-load-more"><button onClick={loadMore} disabled={loadingMore}>{loadingMore ? <LoaderCircle className="spin" size={14}/> : <Plus size={14}/>}<span><b>{loadingMore ? "Loading agreements" : "Load more agreements"}</b><small>{escrows.length - visibleCount} remaining</small></span></button></div>}</>}
    </div>
    ...
    {activeEscrow && <div className="overlay escrow-inspector-overlay"><article className="history-proof-modal escrow-inspector">
      <div className="escrow-inspector-top"><div><span className="section-tag">REFUNDPROTOCOL INSPECTOR</span><small>ARC TESTNET · LIVE CONTRACT RECORD</small></div><button className="modal-x" onClick={() => setActiveEscrow(null)} aria-label="Close inspector"><X size={18}/></button></div>
      <header className="escrow-inspector-header"><div><span className={`escrow-status-pill ${activeEscrow.status}`}><i/>{activeEscrow.status.replaceAll("_", " ").toUpperCase()}</span><h2>{activeEscrow.title}</h2><p>{activeEscrow.specs}</p></div><div className="escrow-reference"><small>PAYMENT REFERENCE</small><b>#{String((activeEscrow.paymentId ?? 0) + 1).padStart(4, "0")}</b><span>Protocol ID {activeEscrow.paymentId ?? 0}</span></div></header>
      <div className="history-proof-summary"><span><small>STATUS</small><b className={activeEscrow.status}>{activeEscrow.status.replaceAll("_", " ").toUpperCase()}</b></span><span><small>VALUE</small><b>{displayMoney(activeEscrow.amount)} USDC</b></span><span><small>BENEFICIARY</small><b>{activeEscrow.providerName}</b></span></div>
      <section className="escrow-inspector-section"><div className="escrow-inspector-section-head"><span className="section-tag">ONCHAIN ROUTE</span><small>Open any address in ArcScan</small></div><div className="escrow-protocol-details">{[["REFUNDPROTOCOL", activeEscrow.contractAddress], ["DEPOSITOR CIRCLE WALLET", activeEscrow.depositorCircleWalletAddress], ["BENEFICIARY CIRCLE WALLET", activeEscrow.beneficiaryCircleWalletAddress], ["BENEFICIARY CONNECTED WALLET", activeEscrow.providerAddress]].map(([label,address]) => <a key={label} href={address ? `https://testnet.arcscan.app/address/${address}` : undefined} target="_blank" rel="noreferrer" className={!address ? "disabled" : ""}><span><small>{label}</small><code>{address ? shortAddress(address, 8) : "Provision pending"}</code></span>{address && <ExternalLink size={13}/>}</a>)}</div></section>
      <section className="escrow-inspector-section"><div className="escrow-inspector-section-head"><span className="section-tag">TRANSACTIONS</span><small>{inspectorTransactions.length} confirmed hash{inspectorTransactions.length === 1 ? "" : "es"}</small></div>{inspectorTransactions.length ? <div className="escrow-tx-list">{inspectorTransactions.map(([label,hash]) => <a key={`${label}-${hash}`} href={`https://testnet.arcscan.app/tx/${hash}`} target="_blank" rel="noreferrer"><span><small>{label}</small><code>{shortAddress(hash, 10)}</code></span><ExternalLink size={13}/></a>)}</div> : <div className="escrow-no-tx">No confirmed transaction hash yet.</div>}</section>
      <section className="escrow-inspector-section"><div className="escrow-inspector-section-head"><span className="section-tag">PROTOCOL AUDIT TRAIL</span><button onClick={() => setShowFullAudit((value) => !value)}>{showFullAudit ? "Show latest" : `View all ${activeEscrow.aiVerificationLogs.length}`}</button></div><div className="escrow-audit-logs">{(showFullAudit ? activeEscrow.aiVerificationLogs : activeEscrow.aiVerificationLogs.slice(-5)).map((log, index) => <div key={`${log}-${index}`} className="audit-log-line"><Bot size={12}/><span>{log}</span></div>)}</div></section>
      <div className="escrow-inspector-evidence">{activeEscrow.deliverableProof && <div className="escrow-proof-box"><small>DELIVERABLE PROOF</small><code>{activeEscrow.deliverableProof}</code></div>}{activeEscrow.validationResult && <div className={`escrow-validation-result ${activeEscrow.validationResult.valid ? "valid" : "invalid"}`}><small>VISION VALIDATION · {activeEscrow.validationResult.confidence}</small><b>{activeEscrow.validationResult.valid ? "Criteria satisfied" : "Evidence rejected"}</b><p>{activeEscrow.validationResult.reasons.join(" · ") || "No unmet criteria reported."}</p><code>sha256:{activeEscrow.validationResult.fileHash}</code></div>}</div>
    </article></div>}
  </section>;
}

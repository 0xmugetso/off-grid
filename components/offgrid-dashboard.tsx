          ...
          {!displayWalletAddress ? (
            ...
            <>
              <section className="real-balances">
                ...
                <article className="real-balance fiat"><div><span className="balance-icon fiat"><Banknote size={16} /></span><small>USER FIAT BALANCE</small><button onClick={() => { void refreshFiatPayouts(); }} aria-label="Refresh fiat balance"><RefreshCw size={13} /></button></div><b>{fiatBalance === null ? "—" : displayMoney(fiatBalance)} <em>USD</em></b><p>{circleMintBalance ? `Circle Mint Pool: ${displayMoney(circleMintBalance.availableUsd)} USD · ${displayMoney(circleMintBalance.unallocatedUsd)} unallocated` : (Number(fiatPending) > 0 ? `${displayMoney(fiatPending)} USD pending` : "Personal account balance")}</p></article>
              </section>

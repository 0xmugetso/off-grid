    ...
    {!walletAddress ? <div className="unified-empty"><Network size={30} /><h2>Connect a wallet to query Gateway.</h2><p>OffGrid will read only real confirmed and pending balances for your connected address.</p><button className="neon-button" onClick={onConnect}><Wallet size={15} /> Connect Wallet</button></div> : <>
      ...
      <div className="unified-chain-grid">{positions.map((position) => {
        ...
        return <article className={!position.queried ? "unlinked" : chainPending > 0 ? "indexing" : ""} key={position.chain}>
          <div className="unified-chain-logo"><ChainLogo chain={position.chain} size={33}/>{supportsFastDeposit(position.chain) && <span className="fast-deposit-hint" tabIndex={0} aria-label="Fast deposits move USDC from this chain into Gateway on Arc Testnet. The spendable balance is shown on the Arc card." title="Fast deposits move USDC from this chain into Gateway on Arc Testnet. The spendable balance is shown on the Arc card."><Zap size={13} /> Fast</span>}</div>
          ...
          {fastDeposited > 0 && <p className="fast-source-total"><span>{displayMoney(fastDeposited)} USDC</span> fast-deposited → Arc</p>}
      ...
      <p className="fast-deposit-note"><Zap size={13} /> OffGrid supports Arc fast deposits from Ethereum Sepolia and Arbitrum Sepolia. Funds arrive in Gateway on Arc.</p>
          ...
          {!displayWalletAddress ? (
            ...
            <>
              <section className="real-balances">
                <div className="balance-intro"><span className="section-tag">REAL TESTNET BALANCES</span><h2>Your money, live.</h2><p>Read directly from connected networks and Circle Gateway. No demo numbers.</p><p className="fast-deposit-note"><Zap size={13} /> Arc fast deposits · Ethereum &amp; Arbitrum Sepolia</p></div>
